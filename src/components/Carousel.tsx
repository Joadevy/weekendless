    ...
    <div className=" w-full overflow-x-auto pb-3 pt-1">
      <ul className="flex animate-carousel gap-4">
        {carouselEvents.map((event, i) => (
          <li
            key={`${event.id}${i}`}
            className="relative aspect-square h-[35vh] max-h-[350px] w-2/3 max-w-[475px] flex-none md:w-1/3 border rounded-md shadow-lg overflow-hidden"
          >
            <Link className="h-full w-full" href={`/event/${event.id}`}>
              <Image
                ...
                src={event.imageUrl}
              />
              <footer className="absolute bottom-0 left-0 w-8/12 p-2 opacity-80 bg-gray-200 flex flex-col gap-1">
